...
path = 'result/'
# loc = [2, 3, 4, 5, 22, 23]
x_train = np.load(path+'x_train.npy')[:,[2,3,4,5]]#.reshape(-1,1)
x_vali = np.load(path+'x_vali.npy')[:,[2,3,4,5]]#.reshape(-1,1)
x_test = np.load(path+'x_test.npy')[:,[2,3,4,5]]#.reshape(-1,1)
...
# plt.hist(y_train)
# plt.yscale('log')
# plt.title('rain_rate_train')
# plt.show()
# # print(kstrain(y_train, 'uniform'))
# # print(stats.shap1iro(y_train))

# plt.hist(x_train[:,2])
# # plt.yscale('log')
# plt.title('z500_train')
# plt.show()
