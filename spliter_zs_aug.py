...
for i in range(20):
    ...
    x_train = np.vstack([x_train, copy_x+np.random.normal(0,1,copy_x.shape)])
    y_train = np.hstack([y_train, copy_y+np.random.normal(0,1,copy_y.shape)])
    
loc_heavy = np.where(y_train>=50)
for i in range(40):
    copy_y = y_train[loc_heavy]
    copy_x = x_train[loc_heavy]
