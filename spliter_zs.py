...
# np.savez_compressed('data_zs_221030_utc8.npz', data)
...
# true = data_rain[:,-1]
# from tools import tp_scatter
# tp_scatter(true, pred, minmax=[0,100])

# loc = np.where((abs(pred-true)/true)<1)
# data_rain_slct = data_rain[loc]
# pred = 0.0576*(10**(data_rain_slct[:,3]/10))**0.557# site，ts，0.5，1.5：0，1，2，3
# true = data_rain_slct[:,-1]
# from tools import tp_scatter
# tp_scatter(true, pred, minmax=[0,100])
#%% 划分降雨等级
# loc=np.where((data[:,-1]>0))
...
data_rain_1 = np.array(random.sample(data_rain_1.tolist(), len(data_rain_2)))
# data_rain_2 = np.array(random.sample(data_rain_2.tolist(), len(data_rain_2)))
