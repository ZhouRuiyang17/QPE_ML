...
# loc = np.where((data[:,-1]<=0) | (data[:,-2]<=0))
# label[loc] = -1

# loc = np.where( ((data[:,-1] - 0.0576*(10**(data[:,-2]/10))**0.557) / data[:,-1] < 0.5) & (data[:,-1] > 0) & (data[:,-2] > 0))
loc = np.where( (abs(data[:,-1] - 0.0576*(10**(data[:,-2]/10))**0.557) / data[:,-1] < 0.5) & (data[:,-1] >= 0) )
label[loc] = 1
#%%
# # data1 = np.array([data[:,:-1], label.reshape(-1,1), data[:,-1]])
# data1 = np.hstack([data[:,:-1], label.reshape(-1,1)])
# data2 = np.hstack([data1, data[:,-1].reshape(-1,1)])
# data = np.array(data2)
