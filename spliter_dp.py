...
data = np.load('dsd_acc3.npy')
loc = np.where(data[:, -1]>0.1) # kdp > 0.1
data_loc = data[loc]
...
# ----划分降雨等级
data_rain_1 = data_loc[data_loc[:,2]<10]
data_rain_2 = data_loc[(data_loc[:,2]>=10) & (data_loc[:,2]<25)]
data_rain_3 = data_loc[(data_loc[:,2]>=25) & (data_loc[:,2]<50)]
data_rain_4 = data_loc[(data_loc[:,2]>=50)]
#%% 划分数据集
...
# ----小雨
x,x1_test,y,y1_test=spliter(data_rain_1[:,-4:-2],data_rain_1[:,2],0.3)
x1_train,x1_vali,y1_train,y1_vali=spliter(x,y,1/7)

# ----中雨
x,x2_test,y,y2_test=spliter(data_rain_2[:,-4:-2],data_rain_2[:,2],0.3)
x2_train,x2_vali,y2_train,y2_vali=spliter(x,y,1/7)

# ----大雨
x,x3_test,y,y3_test=spliter(data_rain_3[:,-4:-2],data_rain_3[:,2],0.3)
x3_train,x3_vali,y3_train,y3_vali=spliter(x,y,1/7)

# ----暴雨
x,x4_test,y,y4_test=spliter(data_rain_4[:,-4:-2],data_rain_4[:,2],0.3)
...
import os
import datetime
dire = 'history/'+str(datetime.datetime.now())[:10]+'_dp/'
